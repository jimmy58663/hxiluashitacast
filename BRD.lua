...
local sets = {
	...
	Idle_Priority = {
        --Main = 'Brass Xiphos',
        Range = 'Ryl.Spr. Horn',
		Head = {'Bard\'s Roundlet', 'Opo-opo Crown', 'Silver hairpin', 'Noble\'s ribbon'},
		Neck = 'Flower Necklace',
		Ear1 = {'Melody Earring', 'Onyx earring'},
		Ear2 = {'Magnetic Earring', 'Melody Earring', 'Onyx earring'},
        Body = {'Sha\'ir Manteel', 'Bard\'s Jstcorps', 'Choral Jstcorps', 'Gaudy Harness', 'Wonder kaftan', 'Beetle harness'},
        ...
        Legs = {'Bard\'s Cannions', 'Choral Cannions', 'Wonder braccae', 'Black Slacks'},
    ...
    Tp_Priority = {
		...
		Back = {'Bard\'s Cape', 'Jester\'s Cape'},
		Waist = {'Swift belt'},
        Legs = {'Byakko\'s Haidate', 'Choral Cannions', 'Wonder braccae', 'Black Slacks'},
        Feet = {'Dusk Ledelsens', 'Errant Pigaches', 'Choral slippers', 'Wonder Clomps'},
    },
	...
	March_Priority = {
		Range = {'Faerie Piccolo', 'Ryl.Spr. Horn'},
	},
...
profile.HandleCommand = function(args)
	...
	if (string.lower(args[1]) == 'minstrel') then
		...
		gFunc.Message(string.format('Minstrel mode %s.', Settings.Minstrel and 'enabled' or 'disabled'));
		return;
	end
end
...
local function PostCast()
	local player = gData.GetPlayer();
	if ((Common.Settings.SubJob == 'WHM' or Common.Settings.SubJob == 'RDM' or Common.Settings.SubJob == 'BLM') and player.MP < 50) then
		Settings.Gaudy = true
	elseif (Common.Settings.SubJob ~= 'WHM' or Common.Settings.SubJob ~= 'RDM' or Common.Settings.SubJob ~= 'BLM' or player.MP >= 50) then
...
profile.HandleMidcast = function()
	...
	if (Common.SupportedSpells:contains(spell.Name)) then
		...
		if (spell.Skill == 'Singing') then
			...
			if (string.contains(spell.Name, 'Paeon') or string.contains(spell.Name, 'Minne') or string.contains(spell.Name, 'Etude')) then
			...
			if (string.contains(spell.Name, 'Threnody') or string.contains(spell.Name, 'Elegy') or string.contains(spell.Name, 'Requiem') or string.contains(spell.Name, 'Lullaby')) then
...
profile.HandleWeaponskill = function()
	...
	local DexWSList = T{'Wasp Sting', 'Gust Slash', 'Viper Bite', 'Cyclone', 'Dancing Edge', 'Shark Bite', 'Evisceration', 'Aeolian Edge', 'Mandalic Stab', 'Rudra\'s Storm', --Dagger
		'Shijin Spiral', --H2H
	...
	if (DexWSList:contains(ws.Name)) then
